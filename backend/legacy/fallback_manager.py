...
def store_mtg_card(card_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        flat_data = {
            ...
            # For double-faced cards (transform/reversible) Scryfall puts images
            # inside card_faces[0].image_uris rather than the top-level field.
            "image_uris_small": (
                card_data.get("image_uris", {}).get("small")
                or ((card_data.get("card_faces") or [{}])[0].get("image_uris") or {}).get("small")
            ),
            "image_uris_normal": (
                card_data.get("image_uris", {}).get("normal")
                or ((card_data.get("card_faces") or [{}])[0].get("image_uris") or {}).get("normal")
            ),
            "image_uris_large": (
                card_data.get("image_uris", {}).get("large")
                or ((card_data.get("card_faces") or [{}])[0].get("image_uris") or {}).get("large")
            ),
            "artist": card_data.get("artist", ""),
        ...
        with open(MTG_CARDS_CSV, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    layout = (row.get('layout') or '').lower()
                    DFC_LAYOUTS = {'transform', 'reversible_card', 'modal_dfc',
                                   'double_faced_token', 'art_series'}
                    is_dfc = layout in DFC_LAYOUTS
                    # Double-faced cards cached before the card_faces fix have no stored
                    # image. Derive one from the Scryfall image API using set+number.
                    if not img and set_code and row.get('collector_number'):
                        img = (
                            f"https://api.scryfall.com/cards/{set_code.lower()}/"
                            f"{row['collector_number']}?format=image&version=normal"
                        )
                    # Back face image URL for DFCs
                    img_back = ''
                    if is_dfc and set_code and row.get('collector_number'):
                        img_back = (
                            f"https://api.scryfall.com/cards/{set_code.lower()}/"
                            f"{row['collector_number']}?format=image&version=normal&face=back"
                        )
                    year = ''
